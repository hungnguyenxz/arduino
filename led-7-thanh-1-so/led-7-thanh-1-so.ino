...
void loop() {
  ...
  display9(); delay(settime);     //Gọi hàm hiển thị số 9 và hiển thị trong khoảng thời gian settime
  display8(); delay(settime);
  display7(); delay(settime);
  display6(); delay(settime);
  display5(); delay(settime);
  display4(); delay(settime);
  display3(); delay(settime);
  display2(); delay(settime);
  display1(); delay(settime);
  display0(); delay(settime);
  allOn(); delay(settime);        //Gọi hàm không hiển thị số trong khoảng thời gian settime
}
//Hiển thị tất cả led
